...
export const getCloudMapper = (mode) => (element, i) => {
  if (element.match(/\n/g)) return React.createElement("br", { key: `${i}-break` })
  if (element === " ") return React.createElement(CloudSpace, { key: `${i}-space` })
  if (mode === "WORD") return React.createElement(CloudWord, { key: `${i}-${element}`, idle: false }, element)
  if (mode === "SPACE") return React.createElement(CloudWord, { key: `${i}-${element}`, idle: true }, element)
...
export const canvasDebug = (ctx, width, height, align, snap, cloudRects) => {
  ...
  const hh = h / snap
  const proceedX = (fn, y) => {
    if (align === "left" || align === "center") for (let x = 0; x <= width; x += hh) fn(x, y)
    else if (align === "right") for (let x = width; x >= 0; x -= hh) fn(x, y)
  }
  const drawLine = (x, y) => {
